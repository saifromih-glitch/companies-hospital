...
class MessageHandler:
    ...
    async def _smart_reply(self, bot, chat_id, response):
        ...
        # Check if response contains CSV/file markers
        if isinstance(response, str) and response.strip():
            # Check for code blocks with file markers
            if "```csv" in response:
                import re
                ...
                if match:
                    ...
                    if "filename:" in response.lower():
                        fn_match = re.search(r'filename:\s*(\S+\.csv)', response, re.I)
                        if fn_match:
                            filename = fn_match.group(1)
                    clean_text = re.sub(r'```csv\s*\n.*?```', '', response, flags=re.DOTALL).strip()
                    if clean_text:
                        await bot.send_message(chat_id, clean_text[:2000])
                    await bot.send_document(chat_id, csv_content, filename, "Generated by Romih Agent")
                    return
            
            # Check for table data that should be a file
            if '"ok": true' in response or '"ok": True' in response:
                import json as j
                try:
                    data = j.loads(response.replace("'", '"'))
